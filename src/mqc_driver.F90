...
module mqc_driver
   ...
   use mqc_method_xtb, only: create_xtb_calculator
#endif
   use mqc_method_hf, only: create_hf_calculator
...
contains
   ...
   subroutine run_calculation(resources, config, sys_geom, bonds, result_out)
      ...
      ! Create calculator based on method type (passed through call chain, no global state)
      call create_calculator(config, calculator)

      ! Log solvation settings if applicable
      if ((allocated(config%solvent) .or. config%dielectric > 0.0_dp) .and. &
      ...
      if (max_level == 0) then
         ...
         call run_unfragmented_calculation(resources%mpi_comms%world_comm, sys_geom, &
                                           calculator, config%calc_type, bonds, config, result_out)
      else
         call run_fragmented_calculation(resources, calculator, config%calc_type, sys_geom, max_level, &
                                         config%allow_overlapping_fragments, &
                                         config%max_intersection_level, bonds, config)
      end if

      ! Clean up calculator
      if (allocated(calculator)) deallocate (calculator)

   end subroutine run_calculation

   subroutine run_unfragmented_calculation(world_comm, sys_geom, calculator, calc_type, bonds, driver_config, result_out)
      ...
      if (calc_type == CALC_TYPE_HESSIAN .and. world_comm%size() > 1) then
         ...
         call distributed_unfragmented_hessian(world_comm, sys_geom, calculator, driver_config)
         return
      end if
      ...
      if (world_comm%size() == 1 .or. world_comm%rank() == 0) then
         ...
         call logger%info(" ")
         call unfragmented_calculation(sys_geom, calculator, calc_type, bonds, result_out)
   ...
   subroutine run_fragmented_calculation(resources, calculator, calc_type, sys_geom, max_level, &
      ...
      call serial_gmbe_pie_processor(pie_atom_sets, pie_coefficients, n_pie_terms, sys_geom, calculator, calc_type, bonds)
         else
            ! Standard MBE serial processing
            call serial_fragment_processor(total_fragments, polymers, max_level, sys_geom, calculator, calc_type, bonds)
         end if
         ...
         call node_worker(resources, sys_geom, calculator, calc_type, bonds)
      end if
  ...
  call logger%error("Rank "//to_char(my_rank)//": Error converting geometry for "//mol_name//": "//error%get_full_trace())
   ...
   subroutine create_calculator(config, calculator)
      !! Create a polymorphic calculator based on method type and configuration
      !!
      !! Factory function that creates the appropriate calculator type (XTB, HF, etc.)
      !! with all settings from the driver configuration applied.
      type(driver_config_t), intent(in) :: config
      class(qc_method_t), allocatable, intent(out) :: calculator
      ...
      case (METHOD_TYPE_GFN1)
         ! Determine solvation model
         if (allocated(config%solvation_model)) then
            solv_model = config%solvation_model
         else
            solv_model = "alpb"
         end if

         ! Create XTB calculator with GFN1
         if (allocated(config%solvent) .or. config%dielectric > 0.0_dp) then
            call create_xtb_calculator("gfn1", calculator, &
                                       solvent=config%solvent, &
                                       solvation_model=solv_model, &
                                       use_cds=config%use_cds, &
                                       use_shift=config%use_shift, &
                                       dielectric=config%dielectric, &
                                       cpcm_nang=config%cpcm_nang, &
      ...
      case (METHOD_TYPE_GFN2)
         ...
         ! Create XTB calculator with GFN2
         if (allocated(config%solvent) .or. config%dielectric > 0.0_dp) then
            call create_xtb_calculator("gfn2", calculator, &
                                       solvent=config%solvent, &
                                       solvation_model=solv_model, &
                                       use_cds=config%use_cds, &
                                       use_shift=config%use_shift, &
      ...
      case (METHOD_TYPE_HF)
         call create_hf_calculator(calculator)

      case default
         call logger%error("Unknown or unsupported method type: "//to_char(config%method))
         error stop "Unsupported method type in create_calculator"
      end select

      call logger%info("Initialized "//method_type_to_string(config%method)//" calculator")

   end subroutine create_calculator
